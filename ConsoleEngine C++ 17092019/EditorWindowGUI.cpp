#include "EditorWindowGUI.h"

EditorCoreGUI::EditorCoreGUI() {};

// Mixing the forgrund and background color
byte EditorCoreGUI::GetColor(byte backgroundColor, byte forgrundColor)
{
	byte color;
...
// Draws window
void EditorCoreGUI::DrawWindow(int xSize, int ySize, int xOffset, int yOffset, bool center, byte backgroundColor, byte forgrundColor)
{
	// Mixing the forgrund and background color
	...
	if (center == true) { xWindow = xMid + xSize / 2; yWindow = yMid + ySize / 2; }
